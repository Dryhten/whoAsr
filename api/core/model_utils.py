    ...
    for item in model_output:
        if isinstance(item, dict):
            ...
            result_item = {
                ...
                "confidence": (
                    float(item[3]) if len(item) > 3 and item[3] is not None else 0.0
                ),
            }
            results.append(result_item)
...
async def process_model_request(
    ...
    try:
        ...
        logger.info(
            f"Processing {model_name.lower()} for audio: {getattr(request, audio_param)}"
        )
        ...
        if text_param:
            ...
            # 对于只需要音频的模型
            model_output = await run_in_threadpool(
                model.generate, input=getattr(request, audio_param)
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"{model_name} processing failed: {str(e)}"
        )
